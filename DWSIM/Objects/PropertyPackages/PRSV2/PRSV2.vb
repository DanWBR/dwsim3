...
Namespace DWSIM.SimulationObjects.PropertyPackages.Auxiliary
    ...
    <DelimitedRecord(vbTab)> <IgnoreFirst()> <System.Serializable()> Public Class PRSV2Param
        ...
        <FieldNullValue(0.0#)> Public kappa2 As Double = 0.0#
        <FieldNullValue(0.0#)> Public kappa3 As Double = 0.0#
    ...
    Public Class PRSV2_IPData
        ...
        Public kji As Double = 0.0#
        Public comment As String = ""

        Public Function Clone() As Object Implements System.ICloneable.Clone

            Dim newclass As New PRSV2_IPData
            With newclass
                .ID1 = Me.ID1
                .ID2 = Me.ID2
                .kij = Me.kij
    ...
    <System.Serializable()> Public Class PRSV2
        ...
        Function CpCvR(ByVal TIPO, ByVal T, ByVal P, ByVal Vz, ByVal VKij, ByVal VKij2, ByVal Vk1, ByVal Vk2, ByVal Vk3, ByVal Vzmass, ByVal VTc, ByVal VPc, ByVal VCpig, ByVal VMM, ByVal Vw, ByVal VZRa)
            ...
            Do
                j = 0
                Do
                    a(i, j) = (ai(i) * ai(j)) ^ 0.5 * (1 - Vz(i) * VKij(i, j) - Vz(j) * VKij2(j, i))
                    j = j + 1
                Loop Until j = n + 1
                i = i + 1
            Loop Until i = n + 1
            ...
            Do
                j = 0
                Do
                    aux2 += Vz(i) * Vz(j) * (1 - Vz(i) * VKij(i, j) - Vz(j) * VKij(j, i)) * (ci(j) * (ai(i) * Tc(j) / Pc(j)) ^ 0.5 + ci(i) * (ai(j) * Tc(i) / Pc(i)) ^ 0.5)
                    j = j + 1
                Loop Until j = n + 1
                i = i + 1
            Loop Until i = n + 1
        ...
        Function Z_PR(ByVal T, ByVal P, ByVal Vx, ByVal VKij, ByVal VKij2, ByVal Vk1, ByVal Vk2, ByVal Vk3, ByVal VTc, ByVal VPc, ByVal Vw, ByVal TIPO)
            ...
            Do
                j = 0
                Do
                    a(i, j) = (ai(i) * ai(j)) ^ 0.5 * (1 - Vx(i) * VKij(i, j) - Vx(j) * VKij2(j, i))
                    j = j + 1
                Loop Until j = n + 1
                i = i + 1
            Loop Until i = n + 1

            Dim sum1(n), sum2(n), sum3(n), aml As Double
            i = 0
            Do
                sum1(i) = 0
                sum2(i) = 0
                ...
                Do
                    If i <> j Then
                        sum2(i) += Vx(i) * Vx(j) * (a(i, i) * a(j, j)) ^ 0.5 * (Vx(j) * VKij2(j, i) - (1 - Vx(i)) * VKij(i, j))
                    End If
                    k = 0
                    Do
                        If i <> j And k > j And k <> i Then
                            sum3(i) += Vx(j) * Vx(k) * (a(j, j) * a(k, k)) ^ 0.5 * (Vx(j) * VKij(j, k) + Vx(k) * VKij2(k, j))
                        End If
                        k += 1
                    Loop Until k = n + 1
                    sum1(i) += Vx(j) * a(i, j)
        ...
        Function H_PR_MIX(ByVal TIPO As String, ByVal T As Double, ByVal P As Double, ByVal Vz As Object, ByVal VKij As Object, ByVal VKij2 As Object, ByVal Vk1 As Object, ByVal Vk2 As Object, ByVal Vk3 As Object, ByVal VTc As Object, ByVal VPc As Object, ByVal Vw As Object, ByVal VMM As Object, ByVal Hid As Double) As Double
            ...
            Do
                j = 0
                Do
                    aux2 += Vz(i) * Vz(j) * (1 - Vz(i) * VKij(i, j) - Vz(j) * VKij2(j, i)) * (ci(j) * (ai(i) * Tc(j) / Pc(j)) ^ 0.5 + ci(i) * (ai(j) * Tc(i) / Pc(i)) ^ 0.5)
                    j = j + 1
                Loop Until j = n + 1
                i = i + 1
            Loop Until i = n + 1
        ...
        Function GeneratePseudoRoot(ByVal T, ByVal P, ByVal Vx, ByVal VKij, ByVal VKij2, ByVal Vk1, ByVal Vk2, ByVal Vk3, ByVal VTc, ByVal VPc, ByVal Vw, ByVal VTb, ByVal TIPO)
            ...
            Do
                ...
                Do
                    j = 0
                    Do
                        a(i, j) = (ai(i) * ai(j)) ^ 0.5 * (1 - Vx(i) * VKij(i, j) - Vx(i) * VKij2(j, i))
                        j = j + 1
                    Loop Until j = n + 1
                    i = i + 1
                Loop Until i = n + 1
